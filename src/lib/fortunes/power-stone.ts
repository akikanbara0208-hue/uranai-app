import { FortuneResult } from "@/lib/types";
import { julianDay, getSunLongitude } from "@/lib/fortunes/astrology";

// ══════════════════════════════════════════════
// パワーストーン占い
// 生まれた日の太陽星座を実計算し、星座に伝統的に対応づけられた守護石を導く。
// （パワーストーン文化は現代スピリチュアルだが、星座と宝石の対応は古くから文書化されている）
// ══════════════════════════════════════════════

type SignStone = {
  sign: string;
  stones: { name: string; effect: string }[];
  theme: string;
  care: string;
  // 4項目（守護石／相性石／テーマ／浄化法）を2〜3文に厚みづけするための星座別の補足
  mainTip: string;
  compatTip: string;
  themeTip: string;
  careTip: string;
};

const SIGN_STONES: SignStone[] = [
  { sign: "牡羊座", theme: "情熱・行動・勝負", care: "月光浴で浄化", stones: [
    { name: "カーネリアン", effect: "やる気と行動力を高め、目標達成を後押しする" },
    { name: "ガーネット", effect: "情熱と持久力を授け、努力を実りに変える" },
    { name: "ルビー", effect: "勝負運とカリスマ性を引き出す" }],
    mainTip: "大事な勝負の日は右手にブレスレットとして身につけると、瞬発力と度胸が高まりやすいでしょう。勝負運も一段と高まります。",
    compatTip: "ガーネットと組み合わせれば粘り強さが加わり、ルビーを添えれば此処一番の華やかさが際立ちます。",
    themeTip: "新しいことを始める朝や挑戦の前にこの石へそっと触れると、迷いを断ち切る勇気が湧いてきます。",
    careTip: "満月の夜に月光浴させると効果的で、週に一度は帰宅後にさっと拭いてホコリを払う習慣もおすすめです。" },
  { sign: "牡牛座", theme: "愛・豊かさ・安定", care: "流水で浄化", stones: [
    { name: "ローズクォーツ", effect: "愛と優しさを育て、人間関係を円満にする" },
    { name: "エメラルド", effect: "豊かさと癒しをもたらし、心を安定させる" },
    { name: "翡翠", effect: "繁栄と健康、徳を積む力を授ける" }],
    mainTip: "寝室やデスクの左側に置いておくと、じんわりと人間関係の緊張がほどけていく感覚を得られるでしょう。心にもゆとりが生まれます。",
    compatTip: "エメラルドは心を落ち着けたい日に、翡翠は金運や健康を意識したい日に合わせると良い相棒になります。",
    themeTip: "焦らず着実に築く恋や仕事の土台を大切にすると、この石が持つ豊かさのエネルギーがじわじわ形になります。",
    careTip: "月に一度、朝の流水にそっとくぐらせるだけで十分で、力任せに擦らずやさしく扱うのがコツです。" },
  { sign: "双子座", theme: "知性・伝達・好奇心", care: "セージで浄化", stones: [
    { name: "シトリン", effect: "明るさと金運、コミュニケーション力を高める" },
    { name: "アゲート（瑪瑙）", effect: "心のバランスを保ち、良い縁を結ぶ" },
    { name: "フローライト", effect: "思考をクリアにし、学びを深める" }],
    mainTip: "商談やプレゼンの前にポケットへ忍ばせておくと、言葉が自然と軽やかに出てくるでしょう。会話の場も和みやすくなります。",
    compatTip: "アゲートは人間関係で迷ったときに、フローライトは勉強や資格の勉強中に身につけると相性抜群です。",
    themeTip: "新しい情報や人との出会いを恐れず飛び込むことで、この石が持つ好奇心のパワーが最大限に発揮されます。",
    careTip: "ホワイトセージの煙を軽くくぐらせるだけで浄化でき、月に2回ほどのペースで行うと本来の輝きが保てます。" },
  { sign: "蟹座", theme: "感情・家庭・癒し", care: "月光浴で浄化", stones: [
    { name: "ムーンストーン", effect: "感情を穏やかに整え、直感を高める" },
    { name: "パール（真珠）", effect: "母性と優しさ、心の平安を授ける" },
    { name: "アクアマリン", effect: "癒しと安心をもたらし、絆を深める" }],
    mainTip: "枕元に置いて眠ると、心の揺らぎがおさまり穏やかな眠りに導いてくれると言われています。安心感にも包まれます。",
    compatTip: "パールは家族や大切な人と過ごす日に、アクアマリンは疲れを感じたときに合わせると癒しの効果が高まります。",
    themeTip: "自分の感情を否定せず受け止めることが、この石のエネルギーと響き合い、身近な人との絆を深めます。",
    careTip: "満月の光を一晩浴びせるのが最も効果的で、雨の日は窓辺で月光浴の代わりに静かに休ませてあげましょう。" },
  { sign: "獅子座", theme: "自信・輝き・創造", care: "日光に短時間あてる", stones: [
    { name: "タイガーアイ", effect: "決断力と洞察力、金運を引き寄せる" },
    { name: "サンストーン", effect: "自信と生命力を高め、存在感を放つ" },
    { name: "ルビー", effect: "情熱とリーダーシップを輝かせる" }],
    mainTip: "人前に立つ日はブレスレットとして左手首に着けると、堂々とした振る舞いを後押ししてくれます。自然と注目も集めやすくなるでしょう。",
    compatTip: "サンストーンは新しい挑戦をする日に、ルビーは自分を強く印象づけたい場面で組み合わせると効果的です。",
    themeTip: "自分の個性を隠さず表現することが、この石が象徴する輝きと創造のエネルギーを引き出す一番の近道です。",
    careTip: "日光浴は10分程度の短時間にとどめ、色あせを防ぐためにも直射日光の当てすぎには注意しましょう。" },
  { sign: "乙女座", theme: "調整・誠実・健康", care: "水晶クラスターで浄化", stones: [
    { name: "ペリドット", effect: "心の曇りを払い、前向きさと健康を保つ" },
    { name: "アマゾナイト", effect: "迷いを整理し、希望へ導く" },
    { name: "サファイア", effect: "誠実さと集中力、知性を高める" }],
    mainTip: "デスク周りに置いておくと、細かい作業や整理整頓に集中しやすい空気をつくってくれるでしょう。作業効率もぐっと上がります。",
    compatTip: "アマゾナイトは考えすぎてしまう夜に、サファイアは集中力を高めたい仕事の場面で心強い味方になります。",
    themeTip: "完璧を求めすぎず、今日できる小さな改善を積み重ねる姿勢が、この石の誠実さと調和していきます。",
    careTip: "水晶クラスターの上に一晩置くだけで浄化でき、月に一度のペースで行うと石の透明感が保たれます。" },
  { sign: "天秤座", theme: "調和・美・人間関係", care: "月光浴で浄化", stones: [
    { name: "ラピスラズリ", effect: "幸運と真実を呼び、邪気を払う" },
    { name: "ローズクォーツ", effect: "愛と美、調和の縁を引き寄せる" },
    { name: "オパール", effect: "魅力と表現力、感性を高める" }],
    mainTip: "大切な人と会う日に身につけると、自然な笑顔と落ち着いた会話がもたらされやすくなります。印象もより柔らかくなるでしょう。",
    compatTip: "ローズクォーツは恋愛や友情を深めたい日に、オパールは自分の魅力を引き出したい場面で合わせると良いでしょう。",
    themeTip: "対立を避けようとするより誠実に自分の意見も伝えることが、この石の持つ真の調和の力を引き出します。",
    careTip: "満月の夜にそっと月光浴させるのがおすすめで、埃がついたら柔らかい布で優しく拭き取りましょう。" },
  { sign: "蠍座", theme: "変容・洞察・守護", care: "セージで浄化", stones: [
    { name: "オブシディアン", effect: "強力に邪気を払い、本質を見抜く力を授ける" },
    { name: "マラカイト", effect: "変容を支え、心身を守る" },
    { name: "トパーズ", effect: "希望と再生、目標達成を後押しする" }],
    mainTip: "外出時にポケットへ入れておくと、不要なエネルギーや邪気から自分を守ってくれると言われています。心も静かに整っていくでしょう。",
    compatTip: "マラカイトは大きな変化の渦中にいる時に、トパーズは新しい目標へ踏み出す時に組み合わせると力になります。",
    themeTip: "物事の表面ではなく本質を見極めようとする姿勢が、この石の持つ深い洞察力と共鳴していきます。",
    careTip: "セージの煙でしっかり浄化した後は、暗く静かな場所でひと晩休ませると力を取り戻しやすくなります。" },
  { sign: "射手座", theme: "拡大・冒険・自由", care: "流水で浄化", stones: [
    { name: "ターコイズ", effect: "旅と挑戦を守り、成功へ導く" },
    { name: "アメジスト", effect: "直感と精神性を高め、心を鎮める" },
    { name: "ラピスラズリ", effect: "幸運と高い視座をもたらす" }],
    mainTip: "旅行や出張の際にお守りとして持ち歩くと、道中の安全と新しい出会いを後押ししてくれるでしょう。移動中の運気も安定します。",
    compatTip: "アメジストは気持ちを落ち着けたい移動中に、ラピスラズリは大きな決断をする場面で組み合わせると心強いです。",
    themeTip: "慣れた場所から一歩踏み出す勇気を持つことが、この石が象徴する自由と拡大のエネルギーを引き寄せます。",
    careTip: "旅先から戻った日は特に、流水でしっかりと浄化してリセットする習慣をつけると良いでしょう。" },
  { sign: "山羊座", theme: "堅実・達成・責任", care: "水晶クラスターで浄化", stones: [
    { name: "ガーネット", effect: "努力を実りに変え、持久力を授ける" },
    { name: "オニキス", effect: "意志を固め、誘惑や邪気から守る" },
    { name: "水晶（クリスタル）", effect: "全体を浄化し、目標達成を支える万能石" }],
    mainTip: "長期的な目標に取り組む期間は肌身離さず身につけると、粘り強さと持久力を支えてくれます。着実な前進も後押しします。",
    compatTip: "オニキスは誘惑に負けそうな時に、水晶は心身をまるごとリセットしたい日に合わせると相性が良いでしょう。",
    themeTip: "地道な努力を軽んじず積み重ねる姿勢こそが、この石が持つ達成と責任のエネルギーを最大限に活かします。",
    careTip: "水晶クラスターに一晩乗せるだけで浄化が完了し、月に一度のケアで石本来の重厚な輝きが戻ります。" },
  { sign: "水瓶座", theme: "独創・自由・革新", care: "月光浴で浄化", stones: [
    { name: "アメジスト", effect: "直感とひらめき、精神の安定をもたらす" },
    { name: "アクアマリン", effect: "自由な発想とコミュニケーションを促す" },
    { name: "フローライト", effect: "独創性と集中力、天才性を引き出す" }],
    mainTip: "新しいアイデアを練る時間に身近に置いておくと、独自の視点やひらめきが降りてきやすくなります。発想もより自由になるでしょう。",
    compatTip: "アクアマリンは人との対話を大切にしたい日に、フローライトは集中して考えをまとめたい時に力を貸します。",
    themeTip: "常識にとらわれず自分らしい発想を大切にすることが、この石の持つ革新のエネルギーと深く共鳴します。",
    careTip: "満月の夜にじっくりと月光浴させると浄化力が高まり、翌朝には澄んだエネルギーを感じられるでしょう。" },
  { sign: "魚座", theme: "直感・共感・癒し", care: "月光浴で浄化", stones: [
    { name: "アクアマリン", effect: "感受性を癒し、慈愛と安らぎを授ける" },
    { name: "アメジスト", effect: "霊性と直感を高め、心を浄化する" },
    { name: "ムーンストーン", effect: "夢と直感を導き、感情を整える" }],
    mainTip: "疲れを感じた日は胸元に近いネックレスとして身につけると、心がすっと軽くなるでしょう。涙もろさも和らいでいきます。",
    compatTip: "アメジストは眠りが浅い夜に、ムーンストーンは感情が揺れやすい時期に合わせると穏やかさを取り戻せます。",
    themeTip: "自分の直感を信じて行動することが、この石が持つ共感と癒しのエネルギーをより豊かに引き出してくれます。",
    careTip: "満月の光の下でひと晩休ませるのが理想的で、疲れを感じた日はこまめに浄化してあげましょう。" },
];

export function getPowerStoneReading(birthday: string): FortuneResult {
  const [y, m, d] = birthday.split("-").map(Number);
  const signIdx = Math.floor(getSunLongitude(julianDay(y, m, d)) / 30) % 12;
  const s = SIGN_STONES[signIdx];
  const main = s.stones[0];

  return {
    title: `あなたの守護石は「${main.name}」`,
    summary: `生まれた日の太陽星座は「${s.sign}」。星座と宝石の対応は古くから文書に残されており、${s.sign}には守護の石が伝統的に定められています。あなたのテーマは「${s.theme}」。この方向を後押ししてくれるパワーストーンをご紹介します。`,
    details: [
      { label: `💎 第一の守護石：${main.name}`, content: `${main.effect}。${s.sign}のあなたを最も力強く支える石です。${s.mainTip}` },
      { label: "✨ 相性のよい石", content: s.stones.slice(1).map((st) => `「${st.name}」── ${st.effect}`).join("\n") + `\n${s.compatTip}` },
      { label: "🌟 守護石のテーマ", content: `${s.sign}の守護石は「${s.theme}」を後押しします。この方向で力を借りると、石のエネルギーとあなたの本質が響き合います。${s.themeTip}` },
      { label: "❤️ 恋愛・対人へのサポート", content: `${s.sign}のテーマ「${s.theme}」は、人との関わりの中でも力を発揮します。守護石「${main.name}」を身につけると、自分らしさを保ちながら良い縁を引き寄せやすくなります。相性のよい石を組み合わせれば、関係に調和とあたたかさが生まれます。` },
      { label: "💼 仕事・金運へのサポート", content: `「${s.theme}」の方向に沿って動くとき、${s.sign}のあなたの運気はもっとも伸びやかになります。守護石「${main.name}」は${main.effect}石として、ここぞという場面であなたを後押しします。焦らず本質を大切にする姿勢が、結果として豊かさを呼び込みます。` },
      { label: "🧼 浄化の方法", content: `おすすめの浄化法は「${s.care}」。パワーストーンは身につけるうちにエネルギーが曇るため、定期的に浄化するとよいとされます。${s.careTip}` },
      { label: "🌙 開運アクション", content: `朝、「${main.name}」をそっと握って「${s.theme}」を一日の意図として込めると、${s.sign}のあなたらしいエネルギーが整いやすくなります。曇りを感じたら「${s.care}」で早めにリセットすることも忘れずに。` },
      { label: "🤝 石とのつき合い方", content: `${s.sign}のあなたには、第一の守護石「${main.name}」だけでなく、「${s.stones.slice(1).map((st) => st.name).join("」「")}」との組み合わせも力になります。身につける・持ち歩く・枕元に置くなど、「${s.theme}」を意識しながら生活に取り入れてみてください。` },
    ],
    lucky: { item: main.name },
    advice: `パワーストーンは、持つ人の意識と響き合ってこそ力を発揮するといわれます。「${main.name}」を完璧なお守りとして崇めるより、本質を思い出させてくれる相棒として、肩の力を抜いてつき合ってみてください。あなた自身が「${s.theme}」を生きようとする一歩こそが、何よりの開運につながります。`,
  };
}
