...
export function getDeityReading(values: Record<string, string>): FortuneResult {
  ...
  return {
    ...
    details: [
      { label: `${deity.icon} ${deity.name}（${deity.title}）の神話`, content: `${deity.myth} この神話に宿る力が、いまあなたの背後に静かに息づいています。` },
      ...
      { label: "💬 守護神からのメッセージ", content: `${deity.name}はあなたにこう語りかけています──「${deity.message}」 この言葉を、これからの人生の道標として心の奥に留めてください。` },
