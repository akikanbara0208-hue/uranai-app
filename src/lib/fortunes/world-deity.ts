...
export function getDeityReading(values: Record<string, string>): FortuneResult {
  ...
  return {
    ...
    summary: `生年月日の数秘術が示す「${domainData.domain}」の領域と、あなたの価値観が引き寄せた${CULTURE_NAMES[culture]}の神話。その交点に宿る守護神は「${deity.name}（${deity.title}）」です。`,
    details: [
      { label: `${deity.icon} ${deity.name}（${deity.title}）`, content: deity.myth },
      { label: "守護神があなたに与える力", content: deity.power },
      { label: "守護神からのメッセージ", content: deity.message },
      { label: `あなたの魂の領域：${domainData.domain}`, content: `生年月日から導き出された魂の数「${domainNum}」は「${domainData.domain}」の領域を示しています。この領域があなたの人生の根底に流れるテーマです。` },
      { label: `${CULTURE_NAMES[culture]}の神話との縁`, content: `あなたの価値観と感性は${CULTURE_NAMES[culture]}の神話世界と深く共鳴しています。${deity.name}はその神話において「${domainData.domain}」を司る存在です。` },
    ],
    lucky: { color: pick(["金色", "銀", "白", "青", "紫", "赤"], r), item: pick(items, r), direction: pick(dirs, r) },
    advice: `${deity.icon} ${deity.name}はあなたの守護神であり、人生の道標です。困難なとき、「${deity.message}」という言葉を胸に刻んでください。守護神への祈りと感謝が、あなたとの縁をさらに深めます。`,
  };
}
